...
public class RenderUtil
{
	...
	public static void renderCenteredWordWrap(PoseStack stack, Font font, FormattedText text, int x, int y, int width, int color)
	{
		List<FormattedCharSequence> texts = font.split(text, width);
		int totalHeight = texts.size() * (font.lineHeight + 2);
		for (int i = 0; i < texts.size(); i++)
			GuiComponent.drawCenteredString(stack, font, texts.get(i), x, y + i * font.lineHeight + 2 - totalHeight / 2, color);
	}
	
	public static void renderHorizontallyCenteredWordWrap(PoseStack stack, Font font, FormattedText text, int x, int y, int width, int color)
	{
		List<FormattedCharSequence> texts = font.split(text, width);
		for (int i = 0; i < texts.size(); i++)
			GuiComponent.drawCenteredString(stack, font, texts.get(i), x, y + i * font.lineHeight + 2, color);
	}
	
	public static void line(PoseStack stack, Vector2f start, Vector2f end, int blitOffset, float lineWidth, float r, float g, float b, float a)
	{
